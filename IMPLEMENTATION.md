...
### Decision 6: Isolated Embedded WebViews (2026-08-21)

**Context**: Embedded guest content must be able to lose its renderer without terminating the parent Wails window or another guest. A shared WebKit context/session can couple process lifetime and storage, which conflicts with that requirement.

**Decision**: Both Linux stacks host guests in a `GtkFixed` overlay above the main webview. Every GTK4 guest receives a unique `WebKitWebContext` and ephemeral `WebKitNetworkSession`; every legacy GTK3 guest receives a unique `WebKitWebContext` backed by an ephemeral `WebKitWebsiteDataManager`. Guests do not register the Wails script-message handler. Popup, download, and unspecified permission requests are denied. `web-process-terminated` marks only the affected guest crashed; explicit `reload()` destroys and recreates that native guest.

**Rationale**:
- Separate WebKit contexts prevent one guest renderer termination from taking the parent or another guest with it.
- Ephemeral per-guest storage makes the isolation guarantee understandable and testable.
- Keeping the Wails bridge out of guests avoids turning arbitrary allowed web content into privileged application code.
- A native overlay provides cross-platform rectangle embedding without coupling guest rendering to the parent DOM renderer.
...
| Embedded guest storage | Ephemeral `WebKitWebsiteDataManager` per guest | Ephemeral `WebKitNetworkSession` per guest |
| Embedded guest crash | Per-guest `web-process-terminated`; explicit reconstruction | Per-guest `web-process-terminated`; explicit reconstruction |
...
v3/pkg/application/
  ...
  embedded_webview_linux.go  # Shared embedded guest lifecycle and events
  embedded_webview_linux_gtk4.go # WebKitGTK 6.0 isolated guest backend
...
v3/pkg/application/
  ...
  embedded_webview_linux.go  # Shared embedded guest lifecycle and events
  embedded_webview_linux_gtk3.go # WebKit2GTK 4.1 isolated guest backend
...
### 2026-08-21
- Added isolated embedded WebView support to the GTK4 default and GTK3 legacy stacks.
- Added a native `GtkOverlay`/`GtkFixed` layer so guest webviews can be positioned over the parent webview without sharing its renderer.
- Added unique ephemeral WebKit context/session state per guest, per-guest crash reporting and reconstruction, navigation/permission policy enforcement, and guest asset requests without Wails runtime access.
- Files: `v3/pkg/application/embedded_webview_linux.go`,
  `v3/pkg/application/embedded_webview_linux_gtk4.go`,
  `v3/pkg/application/embedded_webview_linux_gtk3.go`,
  `v3/pkg/application/linux_cgo.go`, `v3/pkg/application/linux_cgo_gtk3.go`,
  `v3/pkg/application/webview_window_linux.go`, and `v3/internal/assetserver/assetserver.go`.
