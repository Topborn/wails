...
static void embedded_process_terminated(WebKitWebView *view, WebKitWebProcessTerminationReason reason,
                                        gpointer data) {
    EmbeddedIdentity *identity = data;
    goEmbeddedProcessTerminated(identity->window_id, identity->view_id, (int)reason);
}

static gboolean embedded_permission_request(WebKitWebView *view, WebKitPermissionRequest *request,
                                             gpointer data) {
    EmbeddedIdentity *identity = data;
    gboolean microphone = FALSE;
    gboolean camera = FALSE;
    if (WEBKIT_IS_USER_MEDIA_PERMISSION_REQUEST(request)) {
        microphone = webkit_user_media_permission_is_for_audio_device(
            WEBKIT_USER_MEDIA_PERMISSION_REQUEST(request));
        camera = webkit_user_media_permission_is_for_video_device(
            WEBKIT_USER_MEDIA_PERMISSION_REQUEST(request));
    }
    if ((microphone || camera) && goEmbeddedPermissionAllowed(
            identity->window_id, identity->view_id, microphone, camera)) {
...
static gboolean embedded_file_chooser(WebKitWebView *view, WebKitFileChooserRequest *request,
                                      gpointer data) {
...
static void embedded_download_started(WebKitWebContext *context, WebKitDownload *download, gpointer data) {
...
static GtkWidget* create_embedded_webview(GtkFixed *container, guint window_id, guint view_id,
    ...
    g_signal_connect(context, "download-started", G_CALLBACK(embedded_download_started), NULL);
...
static void set_embedded_webview_visible(GtkWidget *widget, gboolean visible) {
...
static void raise_embedded_webview(GtkWidget *widget) {
    ...
    if (window) gdk_window_raise(window);
}
...
func linuxEmbeddedNativeCreate(container pointer, windowID, viewID uint, options embeddedWebViewOptions, policy *EmbeddedWebViewPolicy) (pointer, error) {
	...
	if view == nil {
		return nil, errors.New("create isolated WebKitGTK 4.1 WebView")
	}
...
func linuxEmbeddedNativeSetVisible(view pointer, visible bool) {
	C.set_embedded_webview_visible((*C.GtkWidget)(view), gtkBool(visible))
}
...
func onEmbeddedWebViewProcessRequest(request *C.WebKitURISchemeRequest, windowID, viewID C.guint) {
	...
	webviewRequests <- &webViewAssetRequest{
		...
		windowName:    view.parent.options.Name,
		embeddedGuest: true,
	}
}
